...
const withCarPlayScenesFiles: ConfigPlugin = (config) => {
    return withDangerousMod(config, [
        ...
        async (config) => {
            ...
            fs.copyFile(config.modRequest.projectRoot + "/plugins/carplay/CarSceneDelegate.h", path.join(dir, "CarSceneDelegate.h"));
            fs.copyFile(config.modRequest.projectRoot + "/plugins/carplay/CarSceneDelegate.mm", path.join(dir, "CarSceneDelegate.mm"));
            fs.copyFile(config.modRequest.projectRoot + "/plugins/carplay/SceneDelegate.h", path.join(dir, "SceneDelegate.h"));
            fs.copyFile(config.modRequest.projectRoot + "/plugins/carplay/SceneDelegate.mm", path.join(dir, "SceneDelegate.mm"));
...
const modifySourceFile = async (projectRoot: string, contents: string): Promise<string> => {
    ...
    contents = contents.replace(
        /@implementation AppDelegate/, topAppDelegateMethods)


    contents = contents.replace("return [super application:application didFinishLaunchingWithOptions:launchOptions];", "return YES;");
